...
export default function Home() {
  return (
    <div className="bg-white">
      ...
      {/* ── Hero — fixed height eliminates CLS ───────────────────────────── */}
      <section
        className="bg-[#0f0f0f] text-center flex items-center justify-center"
        style={{ height: "560px", contain: "layout size" }}
      >
      ...
      {/* ── Services — cv-auto skips render until scrolled into view ─────── */}
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {SERVICES.map(({ Icon, name, desc }) => (
              ...
              >
                ...
                <a
                  href={`https://wa.me/919444484399?text=Hi%20Sabari%20Auto%2C%20I%20need%20${encodeURIComponent(name)}`}
                  target="_blank"
                  rel="noreferrer"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {WHY_US.map(({ Icon, title, desc }) => (
              <div
                key={title}
                className="rounded-2xl border border-gray-100 p-6 hover:-translate-y-1 hover:shadow-md transition-all"
              >
                <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-[#ff3b3b]/10">
                  <Icon className="h-5 w-5 text-[#ff3b3b]" />
                </div>
